let quiz = [
  ...
  {
    questions: "Which female artist has the most music videos over 1 Billion Views",
    options: ["Rihanna", "Karol G", "Ariana Grande", "Taylor Swift"],
    correctAnswer: "Rihanna",
  },
  {
    questions: "Which artist has the record for the longest-running number one song",
...
for (let i = 0; i < quiz.length && !cancelled; i++) {
  ...
  if (userAnswer === null) {
    ...
    // Trim whitespaces and make the comparison case-insensitive
    userAnswer = userAnswer.trim().toLowerCase();
    let correctAnswer = quiz[i].correctAnswer.toLowerCase();
